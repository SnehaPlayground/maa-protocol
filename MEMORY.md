...
- **Voice note pipeline**: `node /usr/lib/node_modules/openclaw/node_modules/node-edge-tts/bin.js` with voice `en-IN-NeerjaNeural` (Indian English Female, Microsoft Edge TTS). No API key needed. Wrapper script: `/root/.openclaw/workspace/scripts/voice_edge.js`. Output: WAV/MP3 to `/root/.openclaw/workspace/data/reports/voice_note_YYYY-MM-DD.wav`. Timeout: 120s. All morning reports must also generate a voice note using this pipeline.
