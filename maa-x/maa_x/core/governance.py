...
class GovernanceWrapper:
    """
    Primary entry point wrapping LangGraph (or any callable) with governance guards.

    Parameters
    ----------
    app
        The underlying LangGraph application or callable.
    tenant_context
        Tenant identification and metadata.
    cost_guard
        Budget enforcement.
    canary_router
        Traffic splitting for safe releases.
    approval_gate
    ...
    persistence
        Audit and approval persistence backend.
    metrics
    ...
    def _audit(self, state: Mapping[str, Any], event_type: str, payload: Any) -> None:
        ...
        tenant_id = str(
            ...
            .get("tenant", {})
            .get("tenant_id", state.get("tenant_id", "default"))
        )
